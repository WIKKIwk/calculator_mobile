...
import 'offline_entity_store.dart';
...
class SecureBackupService {
  ...
  static Future<Map<String, dynamic>> _fullBundleForBackup() async {
    final local = await AppLocalStore.snapshotForBackup();
    return {
      'v': 2,
      'exportedAt': DateTime.now().toUtc().toIso8601String(),
      'local': local,
      'entities': OfflineEntityStore.snapshotForBackup(),
    };
  }
  ...
  static Future<Uint8List> buildEncryptedFile(String password) async {
    ...
    final bundle = await _fullBundleForBackup();
  ...
  static Future<void> restoreFromEncryptedBytes(
    ...
    if (v == 2) {
      ...
      if (localRaw is! Map) {
        throw FormatException('Zaxira: local bo\'limi yo\'q.');
      }
      if (entitiesRaw is! Map) {
        throw FormatException('Zaxira: entities bo\'limi yo\'q.');
      }
      final local = Map<String, dynamic>.from(localRaw);
      final entities = Map<String, dynamic>.from(entitiesRaw);
      await AppLocalStore.replaceFromBackupBundle(local);
      await OfflineEntityStore.restoreFromSnapshot(entities);
      return;
    }
    if (v == 1) {
      await AppLocalStore.replaceFromBackupBundle(
        Map<String, dynamic>.from(map),
      );
      await OfflineEntityStore.restoreFromSnapshot({});
      return;
    }
    throw FormatException('Zaxira versiyasi qo\'llab-quvvatlanmaydi (v=$v).');
  }
}
