...
class _UserCalculationScreenState extends State<UserCalculationScreen> {
  ...
  void _openCalcQtySheet(BuildContext context, List<Product> allProducts) {
    ...
    for (var p in selectedProducts) {
      ...
      final text = existingQty != null && existingQty > 0 
          ? (existingQty == existingQty.truncateToDouble() ? existingQty.toInt().toString() : existingQty.toString())
          : '';
  ...
  Future<void> _submitCalculation() async {
    ...
    final itemsInput = _productQuantities.entries.map((e) {
      return {
        'productId': e.key,
        'quantity': e.value,
      };
    ...
    if (result.hasException) {
      final gqlErrs = result.exception?.graphqlErrors ?? [];
      if (gqlErrs.isNotEmpty) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(gqlErrs.first.message)),
        );
        return;
      }

      final lines = <Map<String, dynamic>>[];
      for (final e in _productQuantities.entries) {
        Product? p;
      ...
      await AppLocalStore.addPendingRecordBatch(
        userId: widget.user.id,
        userSnapshot: {
          'id': widget.user.id,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Internet yo\'q yoki server javob bermadi. Ma\'lumot telefonda saqlandi; aloqa tiklanganda avtomatik yuboriladi.',
          ),
        ),
      );
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Kalkulyatsiya saqlandi!')),
    );
    Navigator.pop(context);
  }
      ...
      body: Query(
        ...
        builder: (result, {fetchMore, refetch}) {
          ...
          if (result.hasException && !cached) {
            return Center(child: Text('Xatolik: ${result.exception}'));
          }

          final rawList = result.data?['products'] as List? ?? [];
          final products = rawList.map((e) => Product.fromJson(e as Map<String, dynamic>)).toList();
          _products = products;
          ...
          final totalPrice = _productQuantities.entries.fold<double>(0.0, (sum, entry) {
            final prod = products.cast<Product?>().firstWhere((p) => p?.id == entry.key, orElse: () => null);
            if (prod != null) {
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Text('Umumiy hisob:', style: TextStyle(color: colorScheme.primary, fontWeight: FontWeight.w600, fontSize: 16)),
                    Text('${_formatPrice(totalPrice)} so\'m', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: colorScheme.onSurface)),
                  ],
                ),
              ),
              ...products.map((p) {
                ...
                final qtyStr = existingQty != null && existingQty > 0 
                  ? (existingQty == existingQty.truncateToDouble() ? '${existingQty.toInt()}' : existingQty.toString())
                  : '';
                ...
                return ListTile(
                  ...
                  title: Text(p.name),
                  subtitle: qtyStr.isNotEmpty 
                    ? Text('$qtyStr x ${_formatPrice(p.price)}')
                    : null,
                  trailing: Text('${_formatPrice(p.price)} so\'m', style: TextStyle(color: colorScheme.primary, fontWeight: FontWeight.w600)),
                );
              }).toList(),
            ],
          );
